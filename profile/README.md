<div align="center">
<img 
  src="https://github.com/MeteoraAg/.github/blob/main/meteora/meteora-banner.png?raw=true" 
  style="width:100%; height:100%;"
/>
</div>


# Meteora ☄️

### Building the most dynamic liquidity pools for Liquidity Providers, Launchpads and Launches

## SDKs

- [DAMM V1 SDK](https://github.com/MeteoraAg/dynamic-amm-sdk)
- [DAMM V2 SDK](https://github.com/MeteoraAg/damm-v2-sdk)
- [DLMM SDK](https://github.com/MeteoraAg/dlmm-sdk)
- [Dynamic Bonding Curve SDK](https://github.com/MeteoraAg/ts-sdk)
- [Alpha Vault SDK](https://github.com/MeteoraAg/alpha-vault-sdk)
- [Dynamic Vault SDK](https://github.com/MeteoraAg/vault-sdk)
